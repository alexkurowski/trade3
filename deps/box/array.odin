...
The Array maps a handle to an item. A handle consists of an index and a
...
Example (assumes this package is imported under the alias `hm`):
	...
	entities: hm.Array(ArrayItem, ID, 1024)

	h1 := hm.append(&entities, ArrayItem { pos = { 5, 7 } })
	h2 := hm.append(&entities, ArrayItem { pos = { 10, 5 } })
	...
	h3 := hm.append(&entities, ArrayItem { pos = { 1, 2 } })
...
// Returned from the `append` proc. Store these as permanent references to items in
// the handle map. You can resolve the handle to a pointer using the `get` proc.
ArrayItem :: struct {
  // index into `items` array of the `Array` struct.
  idx: u32,
...
Array :: struct($T: typeid, $HT: typeid, $N: int) {
  // Each item must have a field `id` of type `ArrayItem`.
  //
  // There's always a "dummy element" at index 0. This way, a ArrayItem with
  // `idx == 0` means "no ArrayItem". This means that you actually have `N - 1`
  // items available.
  items:        [N]T,
  ...
  // `unused_items` you'll find the next-next unused index. Used by `append` and
...
clear_array :: proc(m: ^Array($T, $HT, $N)) {
...
append_array :: proc(m: ^Array($T, $HT, $N), v: T) -> (HT, bool) #optional_ok {
  v := v
...
get :: proc(m: ^Array($T, $HT, $N), h: HT) -> ^T {
  if h.idx <= 0 || h.idx >= m.num_items {
    return nil
  }

  if item := &m.items[h.idx]; item.id == h {
    return item
  }
...
remove_array :: proc(m: ^Array($T, $HT, $N), h: HT) {
  if h.idx <= 0 || h.idx >= m.num_items {
    return
  }
...
make_iter :: proc(m: ^Array($T, $HT, $N)) -> Handle_Map_Iterator(T, HT, N) {
  return {m = m, index = 1}
}
...
// `item.id.idx == 0` and in that case skip that item.
